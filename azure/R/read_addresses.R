...
# function: (Combined below with the data transformations.) df <-
# map_df(output[-1], make_df)
# ------------------------------------------------------------------------------
...
write_rds(tmp, paste0(tempdir(), "/property.rds"))
storage_upload(
  cont, 
  src = paste0(tempdir(), "/property.rds"), 
  dest = "property.rds")
